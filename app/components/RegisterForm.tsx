...
const RegisterForm = () => {
  ...
  const [errorrMessage, setErrorMessage] = useState(false);
  ...
  const onSubmit = async (data: Merchant) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://stable-pay-production.up.railway.app/tx/createSplit",
        requestBody,
      );

      await localStorage.setItem(
        "payAccount",
        response.data.split.payingAccount,
      );
  ...
  return (
    <div className="h-full flex-1 py-2 flex items-center justify-center">
      <div className="h-full">
        <h2 className="text-center ">Enter Merchant public address</h2>
        <form onSubmit={handleSubmit(onSubmit)} className="min-w-[400px] px-4">
          <input
            {...register("merchantPublicKey", { required: true })}
            type="text"
            className={` w-full border-2 rounded-lg p-2 outline-none ${errorrMessage ? " border-red-400" : " border-gray-400"}`}
          />
